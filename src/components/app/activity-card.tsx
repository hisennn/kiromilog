...
export function ActivityCard({ activity, variant = "feed" }: ActivityCardProps) {
  ...
  if (variant === "profile") {
    return (
      <article className="activity-card p-3 flex gap-3 items-center rounded-lg border border-line/50 bg-surface/30 overflow-hidden transition-colors hover:bg-surface hover:border-line group">
        <Link href={mediaLink} className="relative aspect-[3/4] w-12 shrink-0 overflow-hidden rounded shadow-sm bg-surface-strong">
          {activity.imageUrl ? (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] leading-snug truncate">
            <span className="text-muted/70 mr-1">{actionText}</span>
            <Link href={mediaLink} className="text-primary font-medium hover:underline">{activity.title}</Link>
          </p>
          <span className="text-[11px] text-muted/50 block mt-1">{activity.relativeTime}</span>
        </div>
      </article>
    );
  }
  ...
  return (
    <article className="activity-card py-3 border-b border-line/30 last:border-0 sm:px-2 rounded-none hover:bg-surface-strong/10 transition-colors">
      <div className="flex gap-3">
        {/* User Avatar */}
        <Link href={`/u/${activity.username}`} className="shrink-0 mt-0.5 group">
          {activity.avatarUrl ? (
            <div className="relative h-8 w-8 overflow-hidden rounded-full ring-2 ring-transparent transition-all group-hover:ring-primary/40">
              <Image alt={`@${activity.username}`} className="object-cover" fill sizes="32px" src={activity.avatarUrl} />
            </div>
          ) : (
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-surface-strong ring-2 ring-transparent transition-all group-hover:ring-primary/40 text-xs font-semibold text-foreground">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-3 mb-2">
            <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
              <Link className="text-sm font-medium text-foreground transition-colors hover:text-primary" href={`/u/${activity.username}`}>
                @{activity.username}
              </Link>
            ...
            <span className="text-[11px] text-muted/50 whitespace-nowrap shrink-0">{activity.relativeTime}</span>   
          </div>

          <div className="flex gap-3 items-center">
            <Link href={mediaLink} className="relative aspect-[3/4] w-11 shrink-0 overflow-hidden rounded bg-surface-strong shadow-sm group">
              {activity.imageUrl ? (
            ...
            <div className="min-w-0 flex flex-col justify-center">
              <Link className="block truncate text-sm font-medium text-primary transition-colors hover:underline" href={mediaLink}>
                {activity.title}
              </Link>
              {activity.mediaKind && (
                <p className="text-[10px] font-medium uppercase tracking-wider text-muted/40 mt-0.5">{activity.mediaKind}</p>
              )}
            </div>
          </div>
        </div>
